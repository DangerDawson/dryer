RSpec.describe Dryer::Cast2 do
  ...
  describe "It can be included without a default module" do
    ...
    it "Properly includes Dryer::Cast::Send" do
      expect(klass.included_modules.any?{ |m| m == Dryer::Cast2 }).to be_truthy
    end

    it "defines the cast macro" do
      expect(klass).to respond_to(:cast2)
    end
    ...
    describe "#cast" do
      ...
      context "arity" do
        context "with arity greater than zero" do
          it "defines the casted method" do
            expect(foobar).to receive(:new).with(param: 1)
      ...
      context "call takes a block" do
        ...
        context "with an arity greater than zero" do
          it "defines the casted method" do
            expect(foobar).to receive(:new).with(param: 1).and_return(foobar_instance)
            expect(foobar_instance).to receive(:call).and_yield(dave)
      ...
      context "param 'with:'" do
        ...
        it "defines the casted method" do
          expect(foobar).to receive(:new).with(method: "value")
          instance.foobar
        end
        ...
        context "single arg" do
          ...
          context "and user specified arg" do
            it "defines the casted method" do
              expect(foobar).to receive(:new).with(method: "value", user: 1)
        ...
        context "hash arg" do
          ...
          context "and user specified arg" do
            it "defines the casted method" do
              expect(foobar).to receive(:new).with(method: instance.local_method, user: 1)
              instance.foobar(user: 1)
            end
          end
        end
        ...
        context "mixed args" do
          ...
          context "and user specified arg" do
            it "defines the casted method" do
              expected_args = { method: instance.method, another_method: instance.local_method }
              expect(foobar).to receive(:new).with(expected_args.merge(user: 1))
              instance.foobar(user: 1)
            end
          end
        end
        ...
        context "no args" do
          ...
          context "and user specified arg" do
            it "defines the casted method" do
              expect(foobar).to receive(:new).with(user: 1)
              instance.foobar(user: 1)
            end
          end
        end
      ...
      context "param 'access:'" do
        ...
        context "public" do
          ...
          it "ensures the cast2 method is public" do
            expect(instance.foobar).to eq :bar
          end
        end
        ...
        context "private" do
          ...
          it "ensures the cast2 method is private" do
    ...
    describe "#config" do
      let(:klass_eval) do
        proc do |args, cast_group_args, cast_args|
          Class.new do
            include Dryer::Cast2.config(args)
            cast_group cast_group_args do
              cast2 :foobar, cast_args
            end
            def method
              "value"
            end
    ...
    describe "#cast_group" do
      ...
      describe "#cast_methods" do
        let(:to) { "Foobar" }
        let!(:cast_group_args) { { with: :aaa } }
        let(:cast_args) { { to: to } }
        context "without specify memoize" do
          it "defines the casted method" do
            expected = { foobar: { to: "Foobar", memoize: false, with: [:aaa] } }
            expect(klass.cast_methods).to eq expected
          end
        end

        context "with specified memoize" do
          let!(:cast_group_args) { { with: :aaa, memoize: true } }
          it "defines the casted method" do
            expected = { foobar: { to: "Foobar", memoize: true, with: [:aaa] } }
            expect(klass.cast_methods).to eq expected
          end
        end
      end
